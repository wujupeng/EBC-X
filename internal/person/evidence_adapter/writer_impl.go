...
type PersonEvidenceWriterImpl struct {
	createdBy string
	gitCommit string
}

func NewPersonEvidenceWriter(createdBy string) *PersonEvidenceWriterImpl {
	return &PersonEvidenceWriterImpl{createdBy: createdBy, gitCommit: getGitCommit()}
}

func (w *PersonEvidenceWriterImpl) Write(
	ctx context.Context, tx *sql.Tx, agg *person.PersonAggregate,
	event person.DomainEvent, mutationType string,
) (*evidence.Record, error) {
	chainID := fmt.Sprintf("person-mutation-chain-%s", agg.TenantID)
