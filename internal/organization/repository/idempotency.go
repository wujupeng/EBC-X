...
func (r *CommandIdempotencyRepositoryPostgreSQL) CheckAndReserve(
	...
	if commandID == "" {
		return nil, organization.ErrOrgCommandIDRequired
	}
	...
	if err == nil {
		...
		case "failed":
			_, _ = tx.ExecContext(ctx, `DELETE FROM business.command_idempotency WHERE command_id = $1 AND tenant_id = $2`, commandID, tenantID)
			return nil, organization.ErrIdempotencyRecordFailed
		case "pending":
			return nil, organization.ErrOrgIdempotencyConflict
		}
	}
...
func (r *CommandIdempotencyRepositoryPostgreSQL) MarkFailed(
	...
	_, err := tx.ExecContext(ctx, `UPDATE business.command_idempotency SET status = 'failed' WHERE command_id = $1 AND tenant_id = $2`, commandID, tenantID)
	if err != nil {
